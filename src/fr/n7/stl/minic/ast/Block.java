...
public class Block {
	...
	public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope) {
		...
		for (Instruction instruction : this.instructions) {
			...
			if (!ok) {
				Logger.error("collect for : " + instruction + " has failed");
				return false;
			}
		}
		return ok;
	...
	public void allocateMemory(Register _register, int _offset) {
		this.length = 0;
		//accumuler les offsets à chaque instruction
		for (Instruction instruction : this.instructions) {
			this.length += instruction.allocateMemory(_register, _offset + this.length);
		}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		for (Instruction instruction : this.instructions) {
			f.append(instruction.getCode(_factory));
		}

		f.add(_factory.createPop(0, length));
		return f;
