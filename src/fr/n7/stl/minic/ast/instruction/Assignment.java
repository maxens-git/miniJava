...
public class Assignment implements Instruction, Expression {
	...
	public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope) {
		if (this.assignable.collectAndPartialResolve(_scope)) {
			return this.value.collectAndPartialResolve(_scope);
		} else {
			Logger.error("Variable : " + this.assignable + " is already defined.");
			return false;
		}
	...
	public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope, FunctionDeclaration _container) {
		return this.collectAndPartialResolve(_scope); // TODO_container
	}
	...
	public boolean completeResolve(HierarchicalScope<Declaration> _scope) {
		Boolean ok = this.value.completeResolve(_scope) && this.assignable.completeResolve(_scope);
		if (!ok) {
			Logger.error("Variable : " + this.value + " is undefined.");
		} 
		return ok;
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		f.add(_factory.createStoreI(this.value.getType().length()));
		return f;
