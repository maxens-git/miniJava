...
public abstract class AbstractField<RecordKind extends Expression> implements Expression {
	...
	public boolean completeResolve(HierarchicalScope<Declaration> _scope) {
		...
		Type nt = this.record.getType();
		if (nt instanceof NamedType) {
			this.field = ((RecordType)((NamedType) nt).getType()).get(this.name);
		} else if (nt instanceof RecordType) {
			while (nt instanceof RecordType) {
				nt = this.record.getType();
			}
			this.field = ((RecordType)((NamedType) nt).getType()).get(this.name);
		}

		if (this.field == null) {
			Logger.error("field not defined");
			return false;
		}
		return ok;
