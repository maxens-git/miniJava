...
public class RecordType implements Type, Declaration, Scope<FieldDeclaration> {
	...
	public RecordType(String _name, Iterable<FieldDeclaration> _fields) {
		...
		for (FieldDeclaration _field : _fields) {
			if (!fieldNames.contains(_field.getName())) {
	...
	public boolean equalsTo(Type _other) {
		...
		if (this.compatibleWith(_other)) {
			...
			for (FieldDeclaration field : this.fields) {
				ok = ok && field.getType() == ((RecordType) _other).get(field.getName()).getType();
			}
		}
		return ok;
	...
	public boolean compatibleWith(Type _other) {
		...
		if (_other instanceof RecordType) {
			...
			for (FieldDeclaration field : this.fields) {
				FieldDeclaration f = other.get(field.getName());
				if (f == null) return false;
				ok = ok && field.getType().compatibleWith(f.getType());
			}
		} else if (_other instanceof SequenceType) {
			SequenceType other = (SequenceType) _other;
			ok = other.compatibleWith(this);
		} else {
			ok = false;
		}
	...
	public void register(FieldDeclaration _declaration) {
		...
		if (this.accepts(_declaration) && !fieldNames.contains(_declaration.getName())) {
	...
	public boolean completeResolve(HierarchicalScope<Declaration> _scope) {
		...
		for (FieldDeclaration f : this.fields) {
			...
			_offset += f.computerOffset(_offset);
		}
		return _result;
	}
